	...
	$('body').on('shown.zui.modal', '#triggerModal', function () {
		...
		if (daterange.length > 0) {
			$.each(daterange, function (i, tag) {
				let id = $(tag).attr('id');
				if (id) {
					form_data['daterange'][id] = $(tag).datetimepicker({
						language: 'zh-CN',
						weekStart: 1,
						todayBtn: 1,
						autoclose: 1,
		...
		if (uploader.length > 0) {
			$.each(uploader, function (i, tag) {
				let id = $(tag).attr('id');
				if (id) {
					form_data['uploader'][id] = $(tag).uploader({
		...
		$(this).parents('.form-group').data('value', $(this).val());
